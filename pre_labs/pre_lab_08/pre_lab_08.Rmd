...
**Answer** There are four total columns and 3,822 rows of data.  
...
**Answer** The line on 57 allows you to rename a new dataset to “balto_911_calls_by_month” while calling the old dataset of “baltco_911_calls.” In line 58, the code is creating a new column for the month. The label=true 

In line 59, the code is grouping the months of the year. In line 60, it is concluding the total number of calls throughout the year. In line 61, it is arranging the calls in order from highest to lowest. 
...
**Answer** After running the code, I see a big blank square of nothing.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** After running the codeblock, I see a bar chart of the call totals in Baltimore County over a year. The x-axis features the months of the year, while y-axis is for the count of calls. 
...
**Answer** After running the code, the calls were arranged in order from highest to lowest(February to August). The x-axis changed from the previous graph, going from month to reorder (month, total_calls). 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Compared to the last codeblock, this new codeblock is horizontal and not vertical as previous graphs showed. Also, the y-axis now features the months of total calls, while x-axis has the count of calls. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The difference is the background is not shaded like previous graph. 
...
**Answer** I noticed that everything remained the same except the background of the graph is colored light blue. 
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** In this graph, I noticed a few differences. One is that there is a title at the top the graph. The other is that there is a name of a source in the bottom right of the graph.  
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** As a result of the codeblock, I see a graph of total number of overdose calls in Baltimore County from February 6. 2022 to February 6, 2023.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** After running code, I see a graph that looks like a EKG monitor. It visually shows total number of calls from April 2022 to January 2023 in a way that’s difficult to read and confirm results. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** After running the codeblock, this graph looks nearly identical to the one in previous question. The difference is the x-axis has dates there, but it’s difficult to read to the dates because it’s concealed by a dark line. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**In this question, the code blocks are producing similar graphs with minor detail. The x-axis starts from January 24, 2022, to February 20, 2023. It’s also counting by a week from the dates based on line 273 of the code.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
