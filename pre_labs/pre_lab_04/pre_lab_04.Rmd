...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** You will see grantee's with the same name, but have a slight difference. For example: WASHINGTON COUNTY COMMUNITY ACTION COUNCIL	and WASHINGTON COUNTY COMMUNITY ACTION COUNCIL INC.
```{r}
...
**Answer**1. The refinr package made the words spelled in customary and all caps, while Grantee to grantee_ is strictly in all caps. I don't see as many duplicates in grantee to grantee_clean juxtaposed to the refinr package.
```{r}
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer**1. Everything is capitalized. 2. In alphabetical order. 3. They're grouped together
...
**Answer** Yes.This one looks in alphabetical order for the letter "W."Some grantees appear twice and/or with subtle differences.
