...
```{r}
journalism_courses <- umd_courses %>% filter(department == "Journalism", seats >= 20)
...
```{r}
courses_2023 <- umd_courses %>% filter(term > 202301)
...
**Answer:** The months are in order from March to June. 
...
**Answer** The values are fluctuating. I think the data represents data from the summer.
...
### Task 11: Better percentage calc2ulation
...
**Answer** The percentage was in order from highest to lowest.
...
**Answer** It's useful because it can help calculate the votes better.
...
**Answer** str_to_title() and str_to_sentence(). The former  converts to title case, where only the first letter of each word is capitalized. str_to_sentence()  convert to sentence case, where only the first letter of sentence is capitalized.
...
**Answer** I think it's organizing the breaking down the data by state name and initials.                     
...
**Answer** No, it doesn't make sense to me because WV is in VA and there's a disparity in their <dbl>.
...
1. What courses have the most seats in the Spring semester?

2. How many seats does JOUR320 have in the Fall semester

3. What JOUR course is taught the most?
