...
**Answer**After running the following code block, I noticed that there’s a data frame featuring dates from 2023. The irony is that the dates are going backward on the first page. For example, The dates start on April 12, 2023, and stop on January 13, 2023. On the second page, it features a title to the html data frame that we loaded earlier in the pre_lab.
```{r}
...
**Answer**After running the following code and looking at the dataframe, I would want to know the time the press release was released. In addition, I would want to know when was the first day they started recording 2023 data. I think it’ll be interesting piece of data to know.
...
**Answer**I believe the output suggests there are 122 tags in the HTML source. 
...
**Answer** There is a numerical drop down list of four social media sites along with what I believe are components that is likely featured on a website. As the long lists continued, you see the date and title of article from the website on the same data entry. 
...
**Answer**The output saw a data frame titled “releases,” which had a numerical lists of social media sites and different tabs that’s likely to belong to a county’s press release. I’m addition, the date is being shown with the month being written alphabetically. 
