...
A2: 2021 has the largest total amount for a fiscal year. I noticed that the first two years are similar then rapidly decreases before making a large jump. I'm curious if these numbers are accurate and indicative of the fiscal year's funds.
...
A3. A Morton Thomas & Assoc INC and A Morton Thomas & Associates INC, Aersol Monitoring & Analysis INC & Aerosol Monitoring & Analysis INC, Ayers Saint Gross & Ayers Saint Gross INC.I would replace the duplicating names so that they're under one name. 	
...
md_grants_loans %>%
  ...
  filter(grantor=="MD Stem Cell Research Fund") %>% 
  summarize(totalamount=sum(amount)) %>% 
    arrange(grantor)
```
